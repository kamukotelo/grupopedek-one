...
import { failureMessage, getSupabaseAdmin, providerLabel, sha256, supabaseRequest, verifyMulticaixaSignature } from './_payments.js';
...
export default async function handler(req, res) {
  ...
  const eventInsert = await supabaseRequest(admin, 'payment_events', {
    ...
    body: JSON.stringify({
      ...
      event_type: event.status === 'SUCCESS' || event.status === 'paid' ? 'emis.payment.success' : 'emis.payment.failed',
    ...
    if (order && order.status !== 'paid') {
      ...
      if (amountMatches && currencyMatches) {
        ...
        await supabaseRequest(admin, `invoices?id=eq.${order.invoice_id}`, {
          method: 'PATCH',
          body: JSON.stringify({ status: 'paid', payment_gateway: providerLabel('multicaixa') }),
        });
  ...
  await supabaseRequest(admin, `payment_events?provider=eq.multicaixa&provider_event_id=eq.${encodeURIComponent(eventId)}`, {
    method: 'PATCH',
