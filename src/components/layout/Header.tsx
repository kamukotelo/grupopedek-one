      ...
      <div className={`hidden lg:block bg-[#030D1F] text-gray-300 text-[11px] font-medium border-b border-white/5 py-2 transition-all ${
        ...
        <div className="container-pepek flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            {/* Logged in User Pill / Portal Button */}
            <button
              type="button"
              onClick={() => setIsPortalOpen(true)}
              className="flex items-center gap-1.5 text-gray-200 hover:text-white font-bold px-3 py-0.5 rounded-full bg-white/10 border border-white/15 hover:bg-[#0B45D8] hover:border-[#0B45D8] transition-all cursor-pointer"
            >
              <User className="w-3 h-3 text-[#0B45D8] group-hover:text-white" />
              <span>{currentUser ? `${currentUser.name.split(' ')[0]} (${currentUser.roleLabel})` : 'Área do Cliente'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 top-[72px] bg-[#06142F]/98 backdrop-blur-2xl z-40 p-6 flex flex-col justify-between border-t border-white/10 overflow-y-auto">
          ...
          <div className="pt-6 border-t border-white/10 space-y-4">
            <button
              type="button"
              onClick={() => { setMobileMenuOpen(false); setIsPortalOpen(true); }}
