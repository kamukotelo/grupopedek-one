...
export const Services: React.FC = () => {
  ...
  return (
    <section id="servicos" className="relative overflow-hidden bg-[#020C1B] py-20 sm:py-28">
      ...
      <div className="container-pepek relative z-10">
        ...
        <div
          className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-4 backdrop-blur-sm sm:p-6"
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <div>
              <span className="text-[10px] font-black uppercase tracking-[0.22em] text-[#D2A820]">{t('servicesCarousel.officialFleet')}</span>
              <p className="mt-1 text-xs text-slate-400">{t('servicesCarousel.autoHint')}</p>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => selectVehicle(activeIndex - 1)} aria-label={t('servicesCarousel.previous')} className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-white transition hover:border-[#D2A820] hover:text-[#D2A820]"><ChevronLeft className="h-5 w-5" /></button>
              <button type="button" onClick={() => selectVehicle(activeIndex + 1)} aria-label={t('servicesCarousel.next')} className="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-white transition hover:border-[#D2A820] hover:text-[#D2A820]"><ChevronRight className="h-5 w-5" /></button>
            </div>
          </div>
          ...
          <div className="mt-4 grid overflow-hidden rounded-3xl border border-white/10 bg-[#06142F] lg:grid-cols-[1.05fr_.95fr]">
            <div className="relative min-h-[300px] bg-[radial-gradient(circle_at_50%_35%,rgba(210,168,32,.16),transparent_55%)] p-8 sm:p-10">
              <img key={activeVehicle.id} src={activeVehicle.primaryImage} alt={activeVehicle.name} className="h-full max-h-[380px] w-full object-contain drop-shadow-[0_30px_30px_rgba(0,0,0,.45)] animate-fadeIn" />
            </div>
            <div className="flex flex-col justify-center border-t border-white/10 p-7 lg:border-l lg:border-t-0 lg:p-10">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 text-xs text-slate-200">
                <span className="flex items-center gap-2 rounded-xl bg-white/5 p-3"><Users className="h-4 w-4 text-[#D2A820]" />{activeVehicle.specs.passengers} {t('servicesCarousel.passengers')}</span>
                <span className="flex items-center gap-2 rounded-xl bg-white/5 p-3"><Luggage className="h-4 w-4 text-[#D2A820]" />{activeVehicle.specs.luggage} {t('servicesCarousel.luggage')}</span>
                <span className="flex items-center gap-2 rounded-xl bg-white/5 p-3"><Gauge className="h-4 w-4 text-[#D2A820]" />{activeVehicle.specs.transmission}</span>
                <span className="flex items-center gap-2 rounded-xl bg-white/5 p-3"><ShieldCheck className="h-4 w-4 text-[#D2A820]" />{activeVehicle.availabilityTag || t('servicesCarousel.onRequest')}</span>
              </div>
