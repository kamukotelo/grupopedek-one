...
expect(stripeWebhookSource.includes('verifyStripeSignature') && stripeWebhookSource.includes('settlePaymentOrder'), 'A liquidação Stripe não depende de webhook assinado');
...
expect(fs.readFileSync('neon/schema.sql', 'utf8').includes('CREATE TABLE IF NOT EXISTS public.payment_events'), 'A trilha de auditoria de pagamentos está ausente');
...
const neonAdminSource = fs.readFileSync('api/_neon.js', 'utf8');
expect(reservationApiSource.includes('getDatabase') && !reservationApiSource.includes('VITE_NEON'), 'Reservas não estão restritas à ligação privada do servidor');
expect(neonAdminSource.includes('DATABASE_URL') && neonAdminSource.includes('jwtVerify'), 'A ligação privada ou validação de sessão Neon está incompleta');
const schemaSource = fs.readFileSync('neon/schema.sql', 'utf8');
expect(schemaSource.includes('REVOKE ALL ON ALL TABLES IN SCHEMA public FROM anonymous, authenticated'), 'A escrita anónima em reservas não foi revogada');
